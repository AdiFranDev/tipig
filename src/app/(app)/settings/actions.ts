...
export async function updateBudgetRatios(formData: FormData) {
...
export async function createScholarshipAllocation(formData: FormData) {
  const supabase = await createClient()
  const {
    data: { user },
  } = await supabase.auth.getUser()
...
export async function deleteScholarshipAllocation(id: string) {
  const supabase = await createClient()
  const {
    data: { user },
  } = await supabase.auth.getUser()
